...
public sealed partial class HomeView : UserControl
{
    ...
    [
        ...
        new("GIS", "GPKG / SHP / GeoTIFF", AppIconKind.Area, false, "开发中"),
        new("BIM", "IFC", AppIconKind.Project, false, "开发中"),
        new("Rhino", "3DM", AppIconKind.View, false, "开发中")
    ];
    ...
    private void ApplyFilter()
    {
        ...
        var filtered = _items.Where(item => MatchesFilter(item, activeFilter) && (string.IsNullOrEmpty(search) || item.DisplayName.Contains(search, StringComparison.OrdinalIgnoreCase) || item.Path.Contains(search, StringComparison.OrdinalIgnoreCase))).ToArray();
    ...
    private string GetActiveFilter() => _activeFilter;

    private static bool MatchesFilter(RecentFile item, string filter) => filter switch
    {
        "CAD" => item.Extension.Equals(".dwg", StringComparison.OrdinalIgnoreCase) || item.Extension.Equals(".dxf", StringComparison.OrdinalIgnoreCase),
        "GIS" => item.Extension.Equals(".gpkg", StringComparison.OrdinalIgnoreCase) || item.Extension.Equals(".shp", StringComparison.OrdinalIgnoreCase) || item.Extension.Equals(".tif", StringComparison.OrdinalIgnoreCase) || item.Extension.Equals(".tiff", StringComparison.OrdinalIgnoreCase),
        "BIM" => item.Extension.Equals(".ifc", StringComparison.OrdinalIgnoreCase),
        "3D" => item.Extension.Equals(".3dm", StringComparison.OrdinalIgnoreCase),
        _ => true
    };
...
{
    public static RecentFileTile From(RecentFile source) => new(source, source.Extension.TrimStart('.').ToUpperInvariant(), source.DisplayName, $"{FormatSize(source.FileSize)} · {FormatWhen(source.LastOpenedUtc)}");
    ...
    private static string FormatWhen(DateTimeOffset openedUtc)
    {
        ...
        if (local.Date == today) return $"今天 {local:HH:mm}";
