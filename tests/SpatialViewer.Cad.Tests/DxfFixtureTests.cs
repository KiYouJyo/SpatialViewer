...
public sealed class DxfFixtureTests
{
    ...
    private static DirectoryInfo FindRoot(DirectoryInfo directory) => File.Exists(Path.Combine(directory.FullName, "SpatialViewer.sln")) ? directory : directory.Parent is { } parent ? FindRoot(parent) : throw new DirectoryNotFoundException("SpatialViewer.sln was not found.");
}
